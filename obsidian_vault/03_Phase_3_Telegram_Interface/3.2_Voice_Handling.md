...
## Description
Add voice message support to the Telegram bot using OpenAI's Whisper API for speech-to-text transcription. Voice is the fastest capture method — a core part of the Capture System. This is the **only module that uses the OpenAI SDK**.
...
## Prompt for Claude
```
You are building **LifeOS Assistant**. Voice is the **fastest capture method** — speaking a quick message is faster than typing. This makes voice support a core part of the Capture System pillar. After transcription, the text flows into the same Claude parser as typed messages.
...
2. Update `src/bot/telegram_bot.py`:
   - Add a voice message handler that:
     ...
     c. Sends the transcribed text back to the user with a prefix: "🎤 I heard: [transcribed text]".
